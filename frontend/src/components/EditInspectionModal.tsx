...
import { X, Calendar, Clock, Settings, CheckCircle, MapPin, Hash, FileText } from 'lucide-react';
import axios from 'axios';

interface EditInspectionModalProps {
  isOpen: boolean;
  onClose: () => void;
  onEditSuccess?: () => void; // Called after successful edit
  inspectionToEdit: any; // Existing inspection object to edit
}

const EditInspectionModal: React.FC<EditInspectionModalProps> = ({ isOpen, onClose, onEditSuccess, inspectionToEdit }) => {
  ...
  // Populate form when modal opens or inspection changes (UI-only changes; logic unchanged)
  useEffect(() => {
    if (inspectionToEdit) {
      setBranch(inspectionToEdit.branch || '');
      setInspectionNo(inspectionToEdit.inspectionNo || inspectionToEdit.inspectionNumber || '');
      setTransformerNo(inspectionToEdit.transformerNo || inspectionToEdit.transformerNumber || '');
      setInspectionDate(inspectionToEdit.inspectionDate || '');
      setInspectionTime(inspectionToEdit.inspectionTime || '');
      setMaintenanceDate(inspectionToEdit.maintenanceDate || '');
      setMaintenanceTime(inspectionToEdit.maintenanceTime || '');
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await axios.patch(
        `http://localhost:8080/api/v1/inspections/${inspectionToEdit.inspectionNo || inspectionToEdit.inspectionNumber}`,
        payload
      );
      ...
      if (axios.isAxiosError(err)) {
        alert(`Failed to update inspection: ${err.response?.data?.message || err.message}`);
      } else {
  ...
  return (
    ...
    >
      <div className="flex justify-center items-center w-full min-h-[50vh]">
        ...
        >
          {/* Header — same gray look as Add modal */}
          <div className="bg-gray-800 px-6 py-4">
            <div className="flex items-center justify-between">
              <div className="flex items-center space-x-3">
                ...
                <div>
                  <h2 className="text-xl font-semibold text-white">Edit Inspection</h2>
                  <p className="text-gray-300 text-sm">Update the inspection details below</p>
                </div>
              </div>
